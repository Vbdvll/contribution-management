...
<div class="dashboard-layout">
    ...
    <main class="dashboard-main">
        <header class="dashboard-header"><div><h1>Générer une amende</h1><p>Associer une pénalité à un membre et une campagne.</p></div></header>
        <section class="content-panel form-panel">
            ...
            <form action="${pageContext.request.contextPath}/admin/amendes/generer" method="post">
                ...
                <div class="mb-3">
                    ...
                    <select name="membreId" class="form-select" required>
                        <c:forEach var="membre" items="${membres}">
                            <option value="${membre.id}" ${membreId == membre.id ? 'selected' : ''}>
                                <c:out value="${membre.numero}"/> - <c:out value="${membre.prenom}"/> <c:out value="${membre.nom}"/>
                            </option>
                        </c:forEach>
                    </select>
                </div>
                ...
                <div class="mb-3"><label class="form-label">Montant de l'amende</label><input type="number" name="montant" class="form-control" value="1000" min="0.01" step="0.01" required></div>
                <div class="form-actions">
                    <button type="submit" class="btn-warning-app">Générer</button>
                    <a href="${pageContext.request.contextPath}/admin/amendes" class="btn-secondary-app">Annuler</a>
                </div>
            </form>
        </section>
    </main>
