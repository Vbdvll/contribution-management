...
public class AmendeDao {
    ...
    public java.math.BigDecimal sommeAmendesPayees() {
        EntityManager em = JpaUtil.getEntityManager();

        try {
            return em.createQuery(
                    "SELECT COALESCE(SUM(a.montant), 0) FROM Amende a WHERE a.statutPaiement = 'PAYEE'",
    ...
    public java.math.BigDecimal sommeAmendesNonPayees() {
        EntityManager em = JpaUtil.getEntityManager();

        try {
            return em.createQuery(
                    "SELECT COALESCE(SUM(a.montant), 0) FROM Amende a WHERE a.statutPaiement = 'NON_PAYEE'",
                    java.math.BigDecimal.class
            ).getSingleResult();
        } finally {
            em.close();
